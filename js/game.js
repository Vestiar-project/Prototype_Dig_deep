...
const MAX_ASCENT_SHARE = 0.1;
const DESCENT_CADENCE_MIN_DEPTH = 300;
const DESCENT_CADENCE_STEP_METERS = METERS_PER_TILE;
const DESCENT_CADENCE_GRACE_SECONDS = 3;
const DESCENT_CADENCE_QUEUE_HOLD_SECONDS = 1.5;
...
const FIELD_ORE_ATLAS_SOURCE = 'assets/field/depth-zero-ores-runtime-atlas.png';
const FIELD_VEIN_ATLAS_SOURCE = 'assets/field/depth-zero-vein-connectors-runtime-atlas.png';
const FIELD_TERRAIN_ATLAS_SIZE = Object.freeze({ width: 1536, height: 1024, cellSize: 512 });
const FIELD_TERRAIN_ATLAS_LAYOUT = Object.freeze({
  loam: Object.freeze({ x: 0, y: 0, width: 512, height: 512 }),
  dirt: Object.freeze({ x: 512, y: 0, width: 512, height: 512 }),
  stone: Object.freeze({ x: 1024, y: 0, width: 512, height: 512 }),
  deepstone: Object.freeze({ x: 0, y: 512, width: 512, height: 512 }),
  bedrock: Object.freeze({ x: 512, y: 512, width: 512, height: 512 }),
  final_seal: Object.freeze({ x: 1024, y: 512, width: 512, height: 512 }),
});
const FIELD_ORE_ATLAS_SIZE = Object.freeze({ width: 1774, height: 887 });
const FIELD_ORE_ATLAS_LAYOUT = Object.freeze({
  copper: Object.freeze({ x: 69, y: 154, width: 286, height: 258 }),
  coal: Object.freeze({ x: 355, y: 149, width: 355, height: 246 }),
  iron: Object.freeze({ x: 710, y: 170, width: 335, height: 225 }),
  amber: Object.freeze({ x: 1130, y: 135, width: 289, height: 290 }),
  silver: Object.freeze({ x: 1419, y: 159, width: 302, height: 239 }),
  gold: Object.freeze({ x: 68, y: 517, width: 264, height: 239 }),
  amethyst: Object.freeze({ x: 460, y: 495, width: 234, height: 244 }),
  prism_crystal: Object.freeze({ x: 790, y: 508, width: 219, height: 229 }),
  void_ore: Object.freeze({ x: 1113, y: 513, width: 306, height: 224 }),
  star_core: Object.freeze({ x: 1419, y: 489, width: 273, height: 283 }),
});
const FIELD_VEIN_ATLAS_SIZE = Object.freeze({ width: 2560, height: 1024, cellSize: 512 });
const FIELD_VEIN_ATLAS_LAYOUT = Object.freeze({
  copper: Object.freeze({ x: 0, y: 0, width: 512, height: 512, portWidthRatio: 0.18 }),
  coal: Object.freeze({ x: 512, y: 0, width: 512, height: 512, portWidthRatio: 0.22 }),
  iron: Object.freeze({ x: 1024, y: 0, width: 512, height: 512, portWidthRatio: 0.20 }),
  amber: Object.freeze({ x: 1536, y: 0, width: 512, height: 512, portWidthRatio: 0.17 }),
  silver: Object.freeze({ x: 2048, y: 0, width: 512, height: 512, portWidthRatio: 0.11 }),
  gold: Object.freeze({ x: 0, y: 512, width: 512, height: 512, portWidthRatio: 0.20 }),
  amethyst: Object.freeze({ x: 512, y: 512, width: 512, height: 512, portWidthRatio: 0.18 }),
  prism_crystal: Object.freeze({ x: 1024, y: 512, width: 512, height: 512, portWidthRatio: 0.18 }),
...
const CAMPAIGN_AUTOBUY_WEIGHTS = Object.freeze({
  ...
  tools_balanced_handle: 0.35,
  tools_pneumatic_pick: 0.16,
  power_sharpened_edge: 0.36,
  power_tempered_steel: 0.34,
  power_furious_swing: 0.2,
  dig_sweeping_arc: 0.24,
  dig_omni_swing: 0.18,
  dig_least_resistance: 0.14,
  dig_mine_lift: 0.08,
  time_extra_breath: 0.28,
  dig_light_footwork: 0.1,
  time_clockwork_heart: 0.2,
  gadgets_scout_drone: 0.16,
  gadgets_drone_battery: 0.24,
  gadgets_drone_drill: 0.22,
  gadgets_drone_swarm: 0.16,
  gadgets_powder_pocket: 0.34,
  sense_echo_pulse: 0.34,
  sense_instinct_spark: 0.3,
  sense_deep_resonance: 0.18,
  sense_panoramic_intuition: 0.25,
  sense_ore_focus: 0.12,
  sense_priority_tuning: 0.2,
  sense_second_fix: 0.16,
  sense_frequency_swing: 0.15,
  sense_far_echo: 0.2,
  power_diamond_tip: 0.24,
  power_sample_calibration: 0.18,
  ...
  tools_mirror_crystal: 0.17,
  tools_super_pick_echo: 0.16,
  power_corebreaker: 0.22,
  sense_earth_call: 0.25,
  dig_quarry_presence: 0.19,
  power_mountain_splitter: 0.28,
  time_thirty_second_oath: 0.22,
  gadgets_demolition_orchestra: 0.31,
  tools_solar_drill: 0.12,
  fortune_motherlode_covenant: 0.16,
  core_bon_voyage: 0.08,
});
  ...
  try {
    ...
    const merged = {
      ...
      workshopBreakthroughTokens: storedVersion >= 13 && Array.isArray(stored.workshopBreakthroughTokens)
        ...
        : [],
      workshopLevelRun: Number.isFinite(Number(stored.workshopLevelRun))
        ? Math.max(-1, Math.floor(Number(stored.workshopLevelRun)))
        : -1,
      workshopLevelsInstalled: Math.max(0, Math.floor(Number(stored.workshopLevelsInstalled) || 0)),
    };
    delete merged.currency;
    delete merged.lifetimeOre;
    delete merged.activeMiningSeconds;
    delete merged.pinnedUpgradeId;
    return merged;
  } catch {
...
function normalizeStats(source = {}) {
  return {
    ...
    mineLiftRecordDepthRatio: clamp(source.mineLiftRecordDepthRatio || 0, 0, 1),
    ...
    deepResonanceEnabled: Boolean(source.deepResonanceEnabled),
    deepResonanceCooldown: Math.max(0, source.deepResonanceCooldown || 0),
    deepResonanceRadiusMultiplier: Math.max(1, source.deepResonanceRadiusMultiplier || 1),
    deepResonanceSolidLayers: Math.max(0, Math.floor(source.deepResonanceSolidLayers || 0)),
    deepResonanceConeHalfAngle: clamp(source.deepResonanceConeHalfAngle || 0, 0, Math.PI * 0.49),
    deepResonanceTargetHold: Math.max(0, source.deepResonanceTargetHold || 0),
    deepResonanceTargetSlots: clamp(Math.floor(source.deepResonanceTargetSlots || 0), 0, 3),
    veinTrailEnabled: Boolean(source.veinTrailEnabled),
    ...
    descentTargetBias: Math.max(0, source.descentTargetBias || 0),
    descentMoveSpeedBonus: Math.max(0, source.descentMoveSpeedBonus || 0),
    openingDescentMoveSpeedBonus: Math.max(0, source.openingDescentMoveSpeedBonus || 0),
    openingDepthPowerBonus: Math.max(0, source.openingDepthPowerBonus || 0),
    ...
    openingSprintMultiplier: Math.max(1, source.openingSprintMultiplier || 1),
    targetRelaySprintMultiplier: Math.max(1, source.targetRelaySprintMultiplier || 1),
    targetRelaySprintDuration: Math.max(0, source.targetRelaySprintDuration || 0),
    faultFinderCadenceEvery: Math.max(0, Math.floor(source.faultFinderCadenceEvery || 0)),
    ...
    solarDrillFinalBurstRadiusTiles: Math.max(0, source.solarDrillFinalBurstRadiusTiles || 0),
  };
}
...
const CAMPAIGN_FINAL_PATH_TARGETS = (() => {
  const targets = new Map();
  const visit = (id, level = 1) => {
    const definition = upgradeById.get(id);
    if (!definition) return;
    const requiredLevel = clamp(Math.floor(Number(level) || 1), 1, definition.maxLevel);
    if ((targets.get(id) || 0) >= requiredLevel) return;
    targets.set(id, requiredLevel);
    for (const requirement of definition.requires || []) {
      if (typeof requirement === 'string') visit(requirement, 1);
      else if (requirement?.id) visit(requirement.id, requirement.level || 1);
    }
  };
  visit(CAMPAIGN.finalUpgrade, 1);
  return targets;
})();
...
function createRunMetrics() {
  return {
    ...
    descentCadenceCompletions: 0,
    descentCadenceTimeouts: 0,
...
const state = {
  ...
  descentCadenceStartedAt: -1,
  descentCadenceGraceRemaining: 0,
  descentCadenceQueued: false,
  descentCadenceQueuedAt: -1,
  descentCadenceQueuedTargetKey: null,
  targetCooldown: 0,
  pathCooldown: 0,
  droneCooldown: 0,
  ping: 0,
  ...
  approachTargetKey: '',
  openingSprintActive: false,
  openingSprintTrailCooldown: 0,
  openingSprintAnnounced: false,
  targetRelaySprintRemaining: 0,
  targetRelaySprintLastTargetKey: '',
...
class SoundEngine {
  constructor() {
    ...
    this.music = window.DepthZeroMusic || null;
    this.lastRockBreakAt = -Infinity;
  }
  ...
  unlock() {
    ...
    const musicState = this.music?.getState?.();
    if (this.music && !musicState?.playing) {
      this.music.start?.({ fadeSeconds: 2.8 })?.catch?.(() => {});
    }
    // The document-level music unlock runs in capture phase. Reuse that
    // context for effects when possible instead of opening two mobile audio
    // sessions for one game.
    if (!this.context) this.context = this.music?.context || new AudioContext();
    if (this.context.state === 'suspended') this.context.resume();
  }

  setEnabled(enabled) {
    this.enabled = Boolean(enabled);
    this.music?.setEnabled?.(this.enabled, {
      persist: true,
      fadeSeconds: this.enabled ? 1.4 : 0.7,
    })?.catch?.(() => {});
  ...
  rockBreak(intensity = 1, source = 'pick') {
    if (!this.enabled) return;
    this.unlock();
    if (!this.context) return;
    const now = this.context.currentTime;
    // One laser pulse can clear a whole row. A short throttle keeps the
    // crumble audible without turning several simultaneous breaks into noise.
    if (now - this.lastRockBreakAt < 0.08) return;
    this.lastRockBreakAt = now;
    const strength = clamp(Number(intensity) || 1, 0.5, 3);
    const laser = source === 'laser';
    this.tone(
      laser ? 168 : 72 + strength * 16,
      laser ? 0.055 : 0.06 + strength * 0.012,
      laser ? 'triangle' : strength >= 1.8 ? 'square' : 'triangle',
      Math.min(0.03, 0.008 + strength * 0.006),
      laser ? -38 : -18,
    );
  }
...
function campaignAutobuyTargetLevel(definitionOrId) {
  const id = typeof definitionOrId === 'string' ? definitionOrId : definitionOrId?.id;
  const bestDepth = Math.max(0, Number(save.bestDepth) || 0);
  const steelReady = getUpgradeLevel(upgradeById.get('tools_steel_pick')) > 0;
  const pneumaticReady = getUpgradeLevel(upgradeById.get('tools_pneumatic_pick')) > 0;
  const superPickReady = getUpgradeLevel(upgradeById.get('tools_super_pick')) > 0;
  const focusReady = getUpgradeLevel(upgradeById.get('sense_ore_focus')) > 0;
  const laserReady = getUpgradeLevel(upgradeById.get('tools_laser_emitter')) > 0;
  const lifetimeOre = (oreId) => Math.max(0, Number(save.lifetimeOres?.[oreId]) || 0);
  const explicitTarget = Math.max(0, CAMPAIGN_AUTOBUY_TARGETS[id] || 0);
  // A purposeful campaign route starts assembling the seven branch summits
  // while the super-pick is crossing the 600–900 m transition. Waiting for
  // the laser made the diagnostic player postpone dozens of inexpensive
  // branch prerequisites until after reaching the bottom, exaggerating a
  // late-game resource drought that a real tree-reading player would avoid.
  const finalPathReady = superPickReady && bestDepth >= 600;
  const finalPathTarget = finalPathReady
    ? Math.max(0, CAMPAIGN_FINAL_PATH_TARGETS.get(id) || 0)
    : 0;
  const target = Math.max(explicitTarget, finalPathTarget);
  if (!target) return 0;

  // The third handle rank is a pneumatic-pick prerequisite, but reserving its
  // eight iron before buying the cheaper steel pick recreates the long
  // copper-only opening. Two handle ranks are enough until steel is installed.
  if (id === 'tools_balanced_handle' && !steelReady) return Math.min(target, 2);
  // Keep the inexpensive first power rank in the opening, then build the
  // critical/impact route only after the pneumatic tool has ended the T1–T3
  // phase. Otherwise four scalar ranks reserve the same iron as the next tool.
  if (id === 'power_sharpened_edge' && !pneumaticReady) return Math.min(target, 1);

  if ([
    'sense_instinct_spark',
    'sense_deep_resonance',
  ].includes(id) && (!pneumaticReady || bestDepth < 400)) return 0;

  if ([
    'sense_panoramic_intuition',
    'sense_far_echo',
  ].includes(id) && (!focusReady || lifetimeOre('gold') <= 0)) return 0;

  if (id === 'sense_ore_focus' && (!pneumaticReady || bestDepth < 650)) return 0;

  if ([
    'sense_priority_tuning',
    'sense_second_fix',
    'power_sample_calibration',
  ].includes(id) && !focusReady) return 0;

  if (id === 'sense_frequency_swing' && (!focusReady || lifetimeOre('prism_crystal') <= 0)) return 0;
  if (id === 'power_corebreaker' && (!superPickReady || lifetimeOre('amethyst') <= 0)) return 0;

  if (CAMPAIGN.capstones.includes(id) && !laserReady) return 0;
  if (id === CAMPAIGN.finalUpgrade) {
    const capstonesReady = CAMPAIGN.capstones.every((upgradeId) => (
      getUpgradeLevel(upgradeById.get(upgradeId)) > 0
    ));
    if (!capstonesReady) return 0;
  }
  return target;
}
...
function getLiftResupplyPreferences(tierCap = 0) {
  ...
  for (const definition of eligibleDefinitions) {
    ...
    for (const [oreId, amount] of Object.entries(recipe)) {
      ...
      const shortage = Math.max(0, amount - (save.inventory?.[oreId] || 0));
      if (shortage <= 0) continue;
      const current = deficits.get(oreId) || {
        oreId,
        votes: 0,
        shortage: 0,
        relativeShortage: 0,
  ...
  return [...deficits.values()]
    .sort((left, right) => (
      ...
      || left.oreId.localeCompare(right.oreId)
    ))
...
function newWorld(seed = (Date.now() ^ Math.floor(Math.random() * 0x7fffffff)) >>> 0, options = {}) {
  ...
  if (
    ...
    if (spawn?.source === 'shaft-lift' && typeof state.world.retuneLiftTarget === 'function') {
      const retunedTarget = state.world.retuneLiftTarget(
        spawn,
        getLiftResupplyPreferences(spawn.requiredTier),
      );
      if (retunedTarget) spawn = { ...spawn, target: retunedTarget };
    }
  }
  state.spawn = {
    ...spawn,
  ...
  state.liftSupplyOreId = spawn.target?.liftSupply ? spawn.target.oreId : null;
  state.player = {
    x: state.spawn.x,
    y: state.spawn.y,
    vx: 0,
...
function startRun(options = {}) {
  ...
  Object.assign(state, {
    ...
    manualPulseCooldown: 0,
    stuckElapsed: 0,
    descentCadenceAnchorDepth: Math.max(state.liftDepth, save.bestDepth || 0),
    descentCadenceStallElapsed: 0,
    descentCadenceGoalDepth: 0,
    descentCadenceStartedAt: -1,
    descentCadenceGraceRemaining: 0,
    ...
    approachTravelElapsed: 0,
    approachTargetKey: '',
    openingSprintActive: (stats.openingSprintMultiplier || 1) > 1,
    openingSprintTrailCooldown: 0,
    openingSprintAnnounced: false,
    targetRelaySprintRemaining: 0,
    targetRelaySprintLastTargetKey: '',
...
function buildRunReport(catalog, haul, activeRunSeconds) {
  ...
  const report = {
    ...
    descentCadenceCompletions: state.metrics.descentCadenceCompletions || 0,
    descentCadenceTimeouts: state.metrics.descentCadenceTimeouts || 0,
    descentCadenceSeconds: Number((state.metrics.descentCadenceSeconds || 0).toFixed(2)),
    descentCadenceEligibleSeconds: Number((state.metrics.descentCadenceEligibleSeconds || 0).toFixed(2)),
...
function updateSoundToggle() {
  ...
  const enabled = save.sound !== false;
  ui.soundToggle.classList.toggle('is-muted', !enabled);
  ui.soundToggle.setAttribute('aria-pressed', String(enabled));
  ui.soundToggle.setAttribute(
    'aria-label',
    enabled
      ? 'Выключить музыку и звуки. Сейчас играет «Полярная жила»'
      : 'Включить музыку и звуки. Саундтрек «Полярная жила»',
  );
  ui.soundToggle.title = enabled
    ? 'Полярная жила · музыка и звуки включены'
    : 'Музыка и звуки выключены';
  if (ui.soundToggleIcon) ui.soundToggleIcon.textContent = enabled ? '♫' : '×';
  if (ui.soundToggleState) ui.soundToggleState.textContent = enabled ? 'ЗВУК' : 'ТИШИНА';
}
...
function renderGeologicalJournal() {
  ...
  for (const ore of ORE_TYPES) {
    ...
    const oreMinDepth = Math.floor((ore.depth || 0) / TILE_SIZE * METERS_PER_TILE);
    const oreMaxDepth = Number.isFinite(Number(ore.maxDepth))
      ? Math.floor(Number(ore.maxDepth) / TILE_SIZE * METERS_PER_TILE)
      : null;
    const oreDepthRange = oreMaxDepth == null
      ? `от ~${oreMinDepth} м`
      : `~${oreMinDepth}–${oreMaxDepth} м`;
    ...
    if (known) {
      card.innerHTML = `
        ...
        <dl>
          ...
          <div><dt>Встречается</dt><dd>${oreDepthRange}</dd></div>
...
function estimateBalanceRun(seed, profilePercent, preparedStats = null) {
  ...
  const randomCriticalChance = expectedChance(simulatedStats.critChance, 0.16);
  const faultCriticalChance = (simulatedStats.faultFinderCadenceEvery || 0) > 0
    ? 1 / simulatedStats.faultFinderCadenceEvery
    : 0;
  const expectedCritical = 1 + (1 - (1 - randomCriticalChance) * (1 - faultCriticalChance))
  ...
  const droneDps = Math.max(0, simulatedStats.droneCount || 0)
    ...
    * simulatedStats.pickPower * Math.max(0, simulatedStats.dronePower || 0.75)
  ...
  const solarDps = simulatedStats.solarDrillEnabled && simulatedStats.solarDrillProcEvery > 0
    ? directDps * (1 + Math.max(0, simulatedStats.solarDrillFinalBurstPower || 0))
      ...
      * Math.max(
        1,
        Math.PI * Math.max(0, simulatedStats.solarDrillFinalBurstRadiusTiles || 0) ** 2 * 0.14,
      )
    : 0;
...
function requirementsMet(definition) {
  ...
  const depthRequirement = Math.max(0, Number(definition.requiresBestDepth) || 0);
  const depthReady = depthRequirement <= 0 || save.bestDepth >= depthRequirement;
  return dependenciesReady && sampleReady && depthReady;
}
...
function resetWorkshopInstallSession() {
  ...
  save.workshopBreakthroughTokens = [];
  save.workshopLevelRun = completedRuns;
  save.workshopLevelsInstalled = 0;
}
...
function workshopLevelStatus() {
  const completedRuns = Math.max(0, Math.floor(Number(save.runs) || 0));
  if (Number(save.workshopLevelRun) !== completedRuns) {
    save.workshopLevelRun = completedRuns;
    save.workshopLevelsInstalled = 0;
    persistSave();
  }
...
function requirementNames(definition) {
  ...
  if ((Number(definition.requiresBestDepth) || 0) > 0) {
    names.push(`рекордная глубина ${Math.floor(definition.requiresBestDepth)} м`);
  }
  return names.join(', ');
}
...
function liftFloorTy() {
  if (!(state.liftDepth > 0) || !state.spawn) return 0;
  return Math.max(0, Math.floor(Number(state.spawn.ty) || 0) - 2);
}

function activeDepthFloorTy() {
  const origin = state.depthOrigin || state.spawn;
  if (!origin) return liftFloorTy();
  const originTy = Number.isFinite(Number(origin.ty))
    ? Math.floor(Number(origin.ty))
    : state.world?.worldToTile(origin.x, origin.y)?.ty || 0;
  const retainedDepthMeters = Math.max(0, Number(state.deepest) || 0) * (1 - MAX_ASCENT_SHARE);
  const retreatFloorTy = originTy + Math.ceil(retainedDepthMeters / METERS_PER_TILE);
  return Math.max(liftFloorTy(), retreatFloorTy);
}

function targetRespectsDepthFloor(target) {
  if (!target) return true;
  return Number(target.ty) >= activeDepthFloorTy();
}
...
function pruneRememberedVeins(focusedOreId = null) {
  ...
  const deepResonanceSlots = Math.max(0, (stats.deepResonanceTargetSlots || 0) - 2);
  const slots = Math.max(seismicSlots, deepResonanceSlots);
  if (slots <= 0) {
  ...
  state.rememberedVeins = state.rememberedVeins.filter((target) => {
    if (seismicSlots <= 0 && !target.deepResonanceRoute) return false;
    ...
    Number(Boolean(right.deepResonanceRoute)) - Number(Boolean(left.deepResonanceRoute))
    || (left.score || Infinity) - (right.score || Infinity)
  )).slice(0, slots);
}

function rememberSeismicRoutes(targets, primary, focusedOreId = null, options = {}) {
  const seismicSlots = Math.max(0, stats.seismicRouteSlots || 0);
  const deepResonanceSlots = options.deepResonance
    ? Math.max(0, (stats.deepResonanceTargetSlots || 0) - 2)
    : 0;
  const slots = Math.max(seismicSlots, deepResonanceSlots);
  if (slots <= 0) return;
  ...
  for (const target of targets) {
    ...
    merged.push({
      ...target,
      rememberedRoute: true,
      deepResonanceRoute: Boolean(options.deepResonance),
    });
  }
...
function findBestOreTargets(x, y, radius, focusedOreId = null, options = {}, limit = 1) {
  ...
  const consider = (tile, tx, ty) => {
    ...
    if (options.respectLiftFloor && !targetRespectsDepthFloor({ ty })) return;
    ...
    const randomCriticalChance = procChance(stats.critChance, 0.16);
    const faultCriticalChance = (stats.faultFinderCadenceEvery || 0) > 0
      ? 1 / stats.faultFinderCadenceEvery
      : 0;
    const expectedCritical = 1 + (1 - (1 - randomCriticalChance) * (1 - faultCriticalChance))
      * Math.max(0, stats.critMultiplier - 1);
    ...
    const laserPower = stats.laserUnlocked
      ? (stats.laserPower || 1) * (1 + Math.max(0, stats.laserPierce || 1) * 0.08)
      : 1;
    const focusedCalibration = focusedOreId ? focusedDamageMultiplier(tile) : 1;
    const openingPower = depthFromOrigin(targetX, targetY) < 100
      ? 1 + (stats.openingDepthPowerBonus || 0)
      : 1;
    const effectivePower = stats.pickPower * openingPower * hardnessPierceMultiplier(tile) * (1 + (stats.oreDamageBonus || 0)) * expectedCritical * expectedMulti * rarePower * laserPower * focusedCalibration;
    const miningSeconds = (tile.hp || 1) / Math.max(0.1, effectivePower * stats.digSpeed);
    const liftSupplyWeight = tile.liftSupply ? 4 : 1;
    // A large late-game sense radius must not dilute the preference for going
    // down. Saturate the comparison after eight rows: a similarly priced node
    // below wins decisively, while an upper node remains available only when
    // it is genuinely much faster or more valuable.
    const verticalTiles = ty - center.ty;
    ...
    const solidLayerLimit = Number.isFinite(Number(options.solidLayerLimit))
      ? Math.max(0, Math.floor(Number(options.solidLayerLimit)))
      : focusedOreId
        ? 7
        : 2;
    if (!options.ignoreSenseLine && !hasSenseLine(x, y, targetX, targetY, solidLayerLimit)) return;
...
function findFrontierReserveTarget(x, y, radius, focusedOreId = null) {
  if (!state.world) return null;
  const currentDepth = depthFromOrigin(x, y);
  const searchRadius = Math.max(radius, TILE_SIZE * 12);
  const center = state.world.worldToTile(x, y);
  const reach = Math.ceil(searchRadius / TILE_SIZE);
  ...
  const consider = (tile, tx, ty) => {
    if (
      !tile?.oreId
      || tile.kind === 'air'
      || tile.frontierReserveOreId !== tile.oreId
      || (save.lifetimeOres?.[tile.oreId] || 0) > 0
      || (focusedOreId && tile.oreId !== focusedOreId)
      || !targetRespectsDepthFloor({ ty })
    ) return;
    const ore = oreById.get(tile.oreId);
    if (!ore) return;
    const thresholdMeters = Math.max(0, Number(ore.depth) || 0)
      / TILE_SIZE
      * METERS_PER_TILE;
    if (currentDepth + 10 < thresholdMeters) return;
    const targetX = (tx + 0.5) * TILE_SIZE;
    const targetY = (ty + 0.5) * TILE_SIZE;
    const dx = targetX - x;
    const dy = targetY - y;
    const distanceSquared = dx * dx + dy * dy;
    if (distanceSquared > searchRadiusSquared) return;
    const score = Math.abs(depthFromOrigin(targetX, targetY) - thresholdMeters) * TILE_SIZE
    ...
    best = {
      ...
      lockRadius: searchRadius,
    };
  };

  if (typeof state.world.forEachOreTileInBounds === 'function') {
    state.world.forEachOreTileInBounds(
      Math.max(0, center.tx - reach),
      Math.max(0, center.ty - reach),
      Math.min(WORLD_CONFIG.WIDTH - 1, center.tx + reach),
      Math.min(WORLD_CONFIG.HEIGHT - 1, center.ty + reach),
      consider,
    );
  } else {
  ...
  best.tile.sensedUntil = Math.max(best.tile.sensedUntil || 0, state.elapsed + 2.5);
  if (!best.tile.frontierReserveSignaled) {
    best.tile.frontierReserveSignaled = true;
    state.shocks.push({
      x,
      y,
      life: 0.58,
    ...
    state.floaters.push({
      x,
      y: y - 40,
      text: 'ЭХО НОВОГО ПЛАСТА',
      color: '#a8fff0',
      life: 0.9,
      maxLife: 0.9,
    });
    sound.tone(132, 0.16, 'sine', 0.025, 220);
  }
  state.ping = Math.max(state.ping || 0, 0.75);
  return best;
}
...
function chooseOreTargets(x, y, radius, focusedOreId = null) {
  ...
  if (trailVeinId) {
    ...
    const veinTargets = findBestOreTargets(
      x,
      y,
      radius * (stats.veinTrailRangeMultiplier || 1),
      focusedOreId,
      {
        veinId: trailVeinId,
  ...
  const ghost = takeGhostTrailTarget(focusedOreId);
  if (ghost && targetRespectsDepthFloor(ghost)) return { primary: ghost, backup: null };
  let remembered = takeRememberedVeinTarget(focusedOreId);
  while (remembered && !targetRespectsDepthFloor(remembered)) {
    remembered = takeRememberedVeinTarget(focusedOreId);
  }
  return { primary: remembered, backup: null };
}
...
function noteTargetAcquired(target) {
  ...
  if (target.kind === 'ore') {
    const changedOreTarget = Boolean(
      state.targetRelaySprintLastTargetKey
      && state.targetRelaySprintLastTargetKey !== key
    );
    if (
      changedOreTarget
      && !state.openingSprintActive
      && (stats.targetRelaySprintDuration || 0) > 0
    ) {
      state.targetRelaySprintRemaining = stats.targetRelaySprintDuration;
      state.floaters.push({
        x: state.player.x,
        y: state.player.y - 34,
        text: 'РЫВОК К НОВОЙ ЖИЛЕ',
        color: '#ffe0a0',
        life: 0.8,
        maxLife: 0.8,
      });
...
function findValuableOreInField(field, focusedOre = null, excludedKeys = []) {
  ...
  const consider = (tile, tx, ty) => {
    ...
    if (!targetRespectsDepthFloor({ ty })) return;
...
function explorationSearchMinTy(centerTy, scanRadius, finalFallback = false) {
  const localMinTy = finalFallback
    ? 0
    : Math.max(0, Math.floor(Number(centerTy) || 0) - Math.floor(Math.max(0, Number(scanRadius) || 0) * 0.35));
  return Math.max(activeDepthFloorTy(), localMinTy);
}

function findExplorationTarget(x, y, focusedOreId = null, options = {}) {
  const center = state.world.worldToTile(x, y);
  const requiredMinTy = clamp(
    Math.floor(Number(options.minimumTy) || 0),
    0,
    WORLD_CONFIG.HEIGHT - WORLD_CONFIG.BEDROCK_ROWS - 1,
  );
  ...
  for (const scanRadius of scanRadii) {
    ...
    const minTy = Math.max(
      explorationSearchMinTy(center.ty, scanRadius, finalFallback),
      requiredMinTy,
    );
    ...
    for (let ty = minTy; ty <= maxTy; ty += 1) {
      for (let tx = minTx; tx <= maxTx; tx += 1) {
        ...
        const upwardPenalty = offsetY < 0 ? 9 + Math.abs(offsetY) * 1.15 : 0;
        const backwardPenalty = forwardOffset < 0 ? 0.9 + Math.abs(forwardOffset) * 0.08 : 0;
        const downwardBias = Math.min(12, Math.max(0, offsetY) * 0.9);
...
function descentCadencePulseDuration() {
  return clamp((stats.runDuration || MIN_RUN_SECONDS) * 0.12, 4, 5);
}

function descentCadenceIsActive() {
  return Number.isFinite(Number(state.descentCadenceStartedAt))
    && Number(state.descentCadenceStartedAt) >= 0
    && (state.descentCadenceGoalDepth || 0) > 0;
}

function liveCurrentOreTarget() {
  if (state.target?.kind !== 'ore') return false;
  const tile = state.world?.getTile(state.target.tx, state.target.ty);
  return Boolean(tile?.oreId && tile.kind !== 'air' && tile.kind !== 'bedrock');
}

function currentOreTargetKey() {
  return liveCurrentOreTarget() ? `${state.target.tx}:${state.target.ty}` : null;
}

function descentCadenceHasPriorityBlocker() {
  if (
    state.target?.kind === 'micro_event'
    || state.target?.motherlode
  ) return true;
  return Boolean(
    findPriorityChestTarget()
    || findMotherlodePriorityTarget()
  );
}

function clearDescentCadenceState(anchorDepth = 0) {
  state.descentCadenceAnchorDepth = Math.max(0, Number(anchorDepth) || 0);
  state.descentCadenceStallElapsed = 0;
  state.descentCadenceGoalDepth = 0;
  state.descentCadenceStartedAt = -1;
  state.descentCadenceGraceRemaining = 0;
  state.descentCadenceQueued = false;
  state.descentCadenceQueuedAt = -1;
  state.descentCadenceQueuedTargetKey = null;
}

function finishDescentCadence(completed = false, timedOut = false) {
  if (completed) state.metrics.descentCadenceCompletions += 1;
  if (timedOut) state.metrics.descentCadenceTimeouts += 1;
  clearDescentCadenceState(state.deepest);
  state.descentCadenceGraceRemaining = DESCENT_CADENCE_GRACE_SECONDS;
  if (state.target?.descentCadence) state.target = null;
  state.pathWaypoint = null;
  state.targetCooldown = 0;
  state.pathCooldown = 0;
}

function beginDescentCadence() {
  const finalApproachDepth = Math.max(0, FINAL_LAYER_DEPTH_METERS - METERS_PER_TILE);
  const deepest = Math.max(0, Number(state.deepest) || 0);
  if (deepest < DESCENT_CADENCE_MIN_DEPTH || deepest >= finalApproachDepth) {
    finishDescentCadence(false, false);
    return false;
  }
  state.descentCadenceGoalDepth = Math.min(
    finalApproachDepth,
    Math.max(deepest, state.descentCadenceAnchorDepth || 0) + DESCENT_CADENCE_STEP_METERS,
  );
  state.descentCadenceStartedAt = state.elapsed;
  state.descentCadenceQueued = false;
  state.descentCadenceQueuedAt = -1;
  state.descentCadenceQueuedTargetKey = null;
  state.target = null;
  ...
  state.targetCooldown = 0;
  state.pathCooldown = 0;
  state.metrics.descentCadencePulses += 1;
  return true;
}

function updateDescentCadence(delta) {
  if (!state.player || !state.world) return;
  const deepest = Math.max(0, Number(state.deepest) || 0);
  const finalApproachDepth = Math.max(0, FINAL_LAYER_DEPTH_METERS - METERS_PER_TILE);
  const anchor = Math.max(0, Number(state.descentCadenceAnchorDepth) || 0);
  const completionDepth = descentCadenceIsActive()
    ? Math.max(0, Number(state.descentCadenceGoalDepth) || 0)
    : anchor + DESCENT_CADENCE_STEP_METERS;

  if (deepest >= completionDepth - 0.25) {
    if (descentCadenceIsActive()) finishDescentCadence(true, false);
    else {
      state.descentCadenceAnchorDepth = deepest;
      state.descentCadenceStallElapsed = 0;
      state.descentCadenceQueued = false;
      state.descentCadenceQueuedAt = -1;
      state.descentCadenceQueuedTargetKey = null;
  ...
  if (deepest >= DESCENT_CADENCE_MIN_DEPTH && deepest < finalApproachDepth) {
    state.metrics.descentCadenceEligibleSeconds += Math.max(0, Number(delta) || 0);
  }

  if (state.target?.kind === 'final_seal' || findFinalSealTarget()) {
    clearDescentCadenceState(deepest);
    return;
  }

  if (descentCadenceHasPriorityBlocker()) {
    if (descentCadenceIsActive()) state.descentCadenceStartedAt += Math.max(0, Number(delta) || 0);
    if (state.descentCadenceQueued) state.descentCadenceQueuedAt += Math.max(0, Number(delta) || 0);
    return;
  }

  if (descentCadenceIsActive()) {
    state.metrics.descentCadenceSeconds += Math.max(0, Number(delta) || 0);
    if (state.elapsed - state.descentCadenceStartedAt >= descentCadencePulseDuration()) {
      finishDescentCadence(false, true);
    }
    return;
  }

  if (state.descentCadenceGraceRemaining > 0) {
    state.descentCadenceGraceRemaining = Math.max(
      0,
      state.descentCadenceGraceRemaining - Math.max(0, Number(delta) || 0),
    );
    return;
  }
  ...
  state.descentCadenceStallElapsed += Math.max(0, Number(delta) || 0);
  if (state.descentCadenceStallElapsed >= descentCadenceStallDelay()) {
    state.descentCadenceQueued = true;
    state.descentCadenceQueuedAt = state.elapsed;
    state.descentCadenceQueuedTargetKey = currentOreTargetKey();
    if (!liveCurrentOreTarget()) beginDescentCadence();
  }
}

function findDescentCadenceTarget(x, y, focusedOreId = null) {
  if (!descentCadenceIsActive()) return null;
  const center = state.world.worldToTile(x, y);
  const origin = state.depthOrigin || state.spawn;
  const originTy = origin
    ? state.world.worldToTile(origin.x, origin.y).ty
    : center.ty;
  const goalTy = originTy + Math.ceil(state.descentCadenceGoalDepth / METERS_PER_TILE);
  const target = findExplorationTarget(x, y, focusedOreId, {
    minimumTy: Math.max(center.ty + 1, goalTy),
  });
  if (!target) {
    finishDescentCadence(false, true);
    return null;
  }
  target.descentCadence = true;
  return target;
}
...
function triggerDeafKnock(x = state.player?.x, y = state.player?.y) {
  ...
  const targets = findBestOreTargets(
    ...
    radius,
    focusedOreId,
    { ignoreSenseLine: true, respectLiftFloor: true },
    (stats.backupTargetSlots || 0) > 0 ? 2 : 1,
  );
...
function triggerDeepResonance(focusedOre = getFocusedOre()) {
  if (
    !state.player
    || !state.world
    || !stats.deepResonanceEnabled
    || !(stats.deepResonanceCooldown > 0)
    || state.deepResonanceCooldownRemaining > 0
  ) return false;

  state.deepResonanceCooldownRemaining = stats.deepResonanceCooldown;
  const radius = effectiveSenseRadius()
    * (stats.deepResonanceRadiusMultiplier || 1)
    * focusedSenseMultiplier(focusedOre);
  const halfAngle = clamp(stats.deepResonanceConeHalfAngle || 0, 0.1, Math.PI * 0.49);
  ...
  const targets = findBestOreTargets(
    ...
    {
      respectLiftFloor: true,
      solidLayerLimit: stats.deepResonanceSolidLayers,
      predicate: (candidate) => {
        const vertical = candidate.y - state.player.y;
        const horizontal = Math.abs(candidate.x - state.player.x);
        return vertical > TILE_SIZE * 0.3 && horizontal <= vertical * coneSlope;
      },
    },
    Math.max(1, stats.deepResonanceTargetSlots || 1),
  );
  ...
  state.shocks.push({
    ...
    maxLife: 0.52,
    tick: Infinity,
    radius: Math.min(radius, effectiveSenseRadius() * 1.15),
    color: '#75bfff',
    kind: 'sense-pulse',
  });
  for (const direction of [-1, 1]) {
    state.beams.push({
      x: state.player.x,
      y: state.player.y,
      x2: state.player.x + Math.sin(halfAngle) * radius * direction,
      y2: state.player.y + Math.cos(halfAngle) * radius,
      color: '#75bfff',
      life: 0.42,
      maxLife: 0.42,
      width: 2,
  ...
  for (const target of targets) {
    target.lockRadius = radius;
    decorateRememberedTarget(target, { echoHold: stats.deepResonanceTargetHold });
  }
  state.target = targets[0];
  state.backupTarget = targets[1] || null;
  rememberSeismicRoutes(
    targets.slice(2),
    state.target,
    focusedOre?.id || null,
    { deepResonance: true },
  ...
  state.floaters.push({
    x: state.player.x,
    y: state.player.y - 40,
    text: targets.length > 1 ? `ГЛУБИННЫЙ ОТКЛИК · ${targets.length} ЦЕЛИ` : 'ГЛУБИННЫЙ ОТКЛИК',
    color: '#a8dcff',
    life: 0.9,
    maxLife: 0.9,
  });
  sound.tone(118, 0.16, 'sine', 0.025, 210);
  return true;
}
...
function findMotherlodePriorityTarget() {
  ...
  if (!target || !targetRespectsDepthFloor(target)) return null;
...
const DIRECT_ROCK_BREAK_SOURCES = new Set(['pick', 'clearance', 'multi', 'laser']);

function spawnRockBreakFx(tile, x, y, source, color) {
  const directToolBreak = DIRECT_ROCK_BREAK_SOURCES.has(source);
  const tier = directToolBreak ? clamp(Math.floor(stats.toolTier || 1), 1, 7) : 1;
  const profile = tier >= 6
    ? { debris: 12, speed: 1.7, size: 1.28, shake: 1.1, sparks: 6, ring: 0.8, intensity: 2.5, accent: '#6fffe4' }
    : tier >= 5
      ? { debris: 9, speed: 1.45, size: 1.2, shake: 2, sparks: 4, ring: 0.62, intensity: 2.1, accent: '#8affdf' }
      : tier >= 4
        ? { debris: 6, speed: 1.25, size: 1.1, shake: 1.2, sparks: 2, ring: 0.45, intensity: 1.55, accent: '#e8c780' }
        : { debris: 4, speed: 1, size: 1, shake: 0.7, sparks: 0, ring: 0, intensity: 0.85, accent: '#c7ae88' };

  spawnDebris(x, y, color, profile.debris, {
    speedMultiplier: profile.speed,
    sizeMultiplier: profile.size,
    glow: tier >= 6,
  });

  if (!directToolBreak) return;
  const fxCooldown = 0.075;
  if (state.elapsed - state.lastRockBreakFxAt < fxCooldown) return;
  state.lastRockBreakFxAt = state.elapsed;
  state.shake = Math.max(state.shake, profile.shake);
  sound.rockBreak(profile.intensity, source);
  if (profile.sparks > 0) spawnSparks(x, y, profile.accent, profile.sparks);
  if (profile.ring > 0) {
    state.shocks.push({
      x,
      y,
      life: tier >= 6 ? 0.24 : 0.18,
      maxLife: tier >= 6 ? 0.24 : 0.18,
      tick: Infinity,
      radius: TILE_SIZE * profile.ring,
...
function resolveBrokenTile(tile, tx, ty, source = 'pick') {
  ...
  const breakColor = ore?.color || (tile?.kind === 'dirt' ? '#74523d' : '#626779');
  if (ore) spawnDebris(x, y, breakColor, 8);
  else spawnRockBreakFx(tile, x, y, source, breakColor);
  ...
  let yieldCount = Math.max(1, Math.floor(Number(ore.baseYield) || 1))
    // The landing sample replaces one ordinary node rather than adding ore
    // density. Its larger yield prevents a deep lift from forcing several
    // return trips for an obsolete shallow-tier recipe.
    + (tile?.liftSupply ? 5 : 0)
    + (noProcSource ? 0 : advanceTripleSample(tile, x, y));
...
function laserTerrainDamageMultiplier(tile) {
  return state.eventSoftRockRemaining > 0 && tile && !tile.oreId ? 1.65 : 1;
}
  ...
  for (let index = 0; index < ricochetCount; index += 1) {
    ...
    state.world.damageRay(
      ...
      onBreak,
      {
        ...
        damageMultiplier: laserTerrainDamageMultiplier,
      },
    );
...
function clearanceTarget(blockedTiles, player) {
  ...
  for (const blocked of blockedTiles) {
    if (
      !blocked?.tile
      || blocked.tile.kind === 'air'
      || blocked.tile.kind === 'bedrock'
      || blocked.tile.kind === 'final_seal'
    ) continue;
...
function attack(aimTarget = state.target) {
  ...
  const faultFinderCritical = (stats.faultFinderCadenceEvery || 0) > 0
    && state.attackCount % stats.faultFinderCadenceEvery === 0;
  const critical = faultFinderCritical || Math.random() < procChance(stats.critChance, 0.16);
  if (faultFinderCritical) state.metrics.faultFinderCadenceCrits += 1;
  // A pick only touches one impact block, so its target can select the event
  // multiplier directly. A laser crosses several materials and delegates the
  // same decision per ray candidate below; applying it here as well would
  // double-boost a beam aimed at ordinary rock.
  const eventSoftnessBonus = state.eventSoftRockRemaining > 0 && !stats.laserUnlocked && !targetOre ? 1.65 : 1;
  ...
  const openingPower = depthFromOrigin(aimTarget.x, aimTarget.y) < 100
    ? 1 + (stats.openingDepthPowerBonus || 0)
    : 1;
  const damage = stats.pickPower * openingPower * streakBonus * densityBonus * oreBonus * rareBonus * chargedBonus * (critical ? stats.critMultiplier : 1) * eventSoftnessBonus * approachBonus * laserHeatBonus;
  if (
    targetOre
    && state.overkillReservoir > 0
    ...
    for (let beamIndex = 0; beamIndex < beamCount; beamIndex += 1) {
      ...
      state.world.damageRay(
        ...
        onBreak,
        {
          damageMultiplier: laserTerrainDamageMultiplier,
          onHit: centralBeam
            ? (tile, tx, ty, appliedDamage) => {
              if (firstLaserImpact) return;
              firstLaserImpact = {
                kind: 'terrain',
                tile,
                tx,
            ...
            : undefined,
          // Splitter rays clear neighboring paths instead of secretly
          // multiplying damage on the selected block.
          excludedKeys: centralBeam ? undefined : new Set([primaryTargetKey]),
        },
      );
  ...
  else if (stats.laserUnlocked && firstLaserImpact) {
    hadToolImpact = true;
  ...
  const directFinalSealImpact = Boolean(
    aimingAtMainTarget
  ...
  const solarImpact = directFinalSealImpact
    ? aimTarget
    : firstLaserImpact || (aimReceivedPrimaryHit ? aimTarget : null);
  if (
    stats.laserUnlocked
    ...
    if (state.solarDrillShotCount % stats.solarDrillProcEvery === 0) {
      ...
      state.solarDrillBursts.push({
        ...
        tx: solarImpact.tx,
        ty: solarImpact.ty,
        finalSeal: directFinalSealImpact,
        remaining: duration,
        maxDuration: duration,
        tick: 0,
  ...
  if (critical || charged) {
    state.floaters.push({
      x: player.x + nx * 45,
      y: player.y + ny * 45,
      text: charged ? 'ЗАРЯД!' : faultFinderCritical ? 'ТРЕЩИНА!' : 'КРИТ!',
      color: charged ? '#8ff7ff' : faultFinderCritical ? '#ffcf72' : '#fff09a',
      life: 0.65,
      maxLife: 0.65,
    });
...
function perkStatusEntries() {
  ...
  if (state.openingSprintActive) add('Стартовый разгон', '+40%', '#ffd790');
  else if (state.targetRelaySprintRemaining > 0) add('Рывок к жиле', '+18%', '#ffe0a0');
  const field = activeMagneticField();
...
function veinTravelSpeedMultiplier(target = state.target) {
  if (!target || !state.player) return 1;
  const movingDeeper = target.y >= state.player.y + TILE_SIZE * 0.4;
  let bonus = movingDeeper ? (stats.descentMoveSpeedBonus || 0) : 0;
  if (movingDeeper && depthFromOrigin(state.player.x, state.player.y) < 100) {
    bonus += stats.openingDescentMoveSpeedBonus || 0;
  }
  if (target.kind !== 'ore' || !state.world) return 1 + bonus;
  const tile = state.world.getTile(target.tx, target.ty);
  const veinId = tile?.veinId || null;
...
function updateSolarDrillBursts(delta) {
  ...
  for (const burst of state.solarDrillBursts) {
    ...
    if (burst.remaining <= 0) {
      ...
      if (finalPower > 0) {
        const finalRadius = TILE_SIZE * Math.max(
          1.15,
...
function updateRun(delta, now = performance.now()) {
  ...
  state.deepResonanceCooldownRemaining = Math.max(0, state.deepResonanceCooldownRemaining - delta);
  state.openingSprintTrailCooldown = Math.max(0, state.openingSprintTrailCooldown - delta);
  state.targetRelaySprintRemaining = Math.max(0, state.targetRelaySprintRemaining - delta);
  ...
  if (state.target) {
    ...
    if (
      ...
      || (!microEventTarget && !finalSealTarget && !targetRespectsDepthFloor(state.target))
  ...
  if (state.targetCooldown <= 0) {
      ...
      : priorityMotherlode
        ...
        : incumbentOre
          ? {
            ...
            backup: targetRespectsDepthFloor(state.backupTarget)
              && oreTargetIsValid(state.backupTarget, focusedOre?.id || null)
              ? state.backupTarget
              : null,
          }
    ...
    else if (triggerDeepResonance(focusedOre)) {
      refreshCrewBeacon(state.target);
    }
  ...
  if (state.target) {
    if (
      ...
      const route = state.world.findLeastResistanceStep(state.player, state.target, {
        ...
        digPowerPerSecond: stats.pickPower
          * stats.digSpeed
          * (depthFromOrigin(state.player.x, state.player.y) < 100
            ? 1 + (stats.openingDepthPowerBonus || 0)
            : 1),
    ...
    const activeFloorTy = activeDepthFloorTy();
    if (state.pathWaypoint && Number(state.pathWaypoint.ty) < activeFloorTy) {
      state.pathWaypoint = null;
      state.pathCooldown = 0;
    }
    ...
    const sprintMultiplier = state.openingSprintActive
      ? stats.openingSprintMultiplier
      : state.targetRelaySprintRemaining > 0
        ? stats.targetRelaySprintMultiplier
        : 1;
    ...
    const retreatFloorY = (activeFloorTy + 0.5) * TILE_SIZE;
    const nextY = clamp(
      Math.max(retreatFloorY, state.player.y + ny * moveDistance),
      TILE_SIZE,
      WORLD_CONFIG.HEIGHT * TILE_SIZE - TILE_SIZE,
    );
    const perpendicularX = -ny;
    const perpendicularY = nx;
    ...
    if (!blockedTiles.length) {
      ...
      if ((state.openingSprintActive || state.targetRelaySprintRemaining > 0) && moveDistance > 0.1) {
        if (!state.openingSprintAnnounced) {
          state.openingSprintAnnounced = true;
          state.floaters.push({
            x: state.player.x,
            y: state.player.y - 34,
            text: 'СТАРТОВЫЙ РАЗГОН',
            color: '#ffd790',
            life: 0.9,
            maxLife: 0.9,
          });
        }
        if (state.openingSprintTrailCooldown <= 0) {
          spawnOpeningSprintTrail(state.player.x, state.player.y, nx, ny);
          state.openingSprintTrailCooldown = 0.08;
        }
      }
  ...
  if (!state.target) {
    ...
    state.metrics.miningSeconds += delta;
  }
...
function droneAttack() {
  ...
  const playerTargetKey = state.target?.kind === 'ore'
    ? `${state.target.tx}:${state.target.ty}`
    : null;
  const backupTargetKey = state.backupTarget?.kind === 'ore'
    ? `${state.backupTarget.tx}:${state.backupTarget.ty}`
    : null;
  for (let index = 0; index < count; index += 1) {
    ...
    const fieldRangeMultiplier = activeMagneticField() ? 1 + stats.magneticFieldTargetingBonus : 1;
    const baseRange = (
      effectiveSenseRadius() * 1.35
      + TILE_SIZE
      + Math.max(0, (stats.pickupRadius || 46) - 46) * 0.35
    )
      * (focusedOre ? focusedSenseMultiplier(focusedOre) : 1)
      * (triangle ? 1 + stats.triangularFixRangeBonus : 1)
      * fieldRangeMultiplier;
    const selectTarget = (excludedKeys) => {
      const commonOptions = {
        excludedKeys,
        solidLayerLimit: 5,
        respectLiftFloor: true,
      ...
      const triangleTarget = (triangle || hasRememberedOre) && stats.triangularFixDronePriority
        ...
        : null;
      const magneticTarget = findMagneticFieldTarget(focusedOre, excludedKeys);
      return magneticTarget || triangleTarget || findBeaconAwareTarget(
        origin.x,
        origin.y,
        baseRange,
        focusedOre,
    ...
    const preferredExclusions = new Set(claimedKeys);
    if (playerTargetKey) preferredExclusions.add(playerTargetKey);
    if (backupTargetKey) preferredExclusions.add(backupTargetKey);
    let target = selectTarget(preferredExclusions);
    if (!target && playerTargetKey) target = selectTarget(claimedKeys);
    if (!target && claimedKeys.size) target = selectTarget(new Set());
    if (!target) continue;
    claimedKeys.add(`${target.tx}:${target.ty}`);
    if (target.tile) {
      target.tile.discovered = true;
      target.tile.sensedUntil = Math.max(target.tile.sensedUntil || 0, state.elapsed + 0.9);
    }
    const inTriangle = pointInTriangle(target.x, target.y, triangle);
    const gadgetBonus = inTriangle ? stats.triangularFixGadgetDamageBonus : 0;
    if (inTriangle || gadgetBonus > 0 || (triangle && stats.triangularFixRangeBonus > 0)) state.metrics.triangleBuffHits += 1;
    const targetOre = oreById.get(target.tile?.oreId);
    const orePower = 1 + Math.max(0, stats.oreDamageBonus || 0) * 0.7;
    const rarePower = 1 + (oreRank(targetOre) >= 4 ? Math.max(0, stats.rareOreDamageBonus || 0) * 0.7 : 0);
    const power = stats.pickPower * Math.max(0.2, stats.dronePower || 0.75) * (1 + gadgetBonus)
      * magneticGadgetMultiplier(target.x, target.y)
      * hardnessPierceMultiplier(target.tile)
      * orePower
      * rarePower;
    const hpBefore = target.tile.hp || 0;
    const comboTarget = { ...target, veinId: target.tile?.veinId || null };
...
function spawnDebris(x, y, color, count, options = {}) {
  const particleCount = REDUCED_MOTION ? Math.max(1, Math.ceil(count * 0.55)) : count;
  const speedMultiplier = clamp(Number(options.speedMultiplier) || 1, 0.5, 2.5);
  const sizeMultiplier = clamp(Number(options.sizeMultiplier) || 1, 0.6, 2);
  for (let index = 0; index < particleCount; index += 1) {
    const angle = Math.random() * Math.PI * 2;
    const speed = (28 + Math.random() * 92) * speedMultiplier;
    state.particles.push({
      x,
      y,
      vx: Math.cos(angle) * speed,
...
function spawnOpeningSprintTrail(x, y, nx, ny) {
  const count = REDUCED_MOTION ? 1 : 2;
  for (let index = 0; index < count; index += 1) {
    const side = (index - (count - 1) * 0.5) * 9;
    state.particles.push({
      x: x - nx * 11 - ny * side,
      y: y - ny * 11 + nx * side + 9,
      vx: -nx * (28 + index * 7) - ny * side * 1.6,
      vy: -ny * (28 + index * 7) + nx * side * 1.6 - 8,
      size: 3.2 + index * 0.8,
      color: index % 2 ? '#ffd98e' : '#b7e5ff',
      life: 0.28,
      maxLife: 0.28,
      gravity: 24,
      glow: true,
...
function horizontalCameraBounds(worldWidth, viewportWidth) {
  const worldExtent = Math.max(0, Number(worldWidth) || 0);
  const viewportExtent = Math.max(0, Number(viewportWidth) || 0);
  const scrollableWidth = worldExtent - viewportExtent;
  if (scrollableWidth >= 0) return { min: 0, max: scrollableWidth };
  const centeredOffset = scrollableWidth * 0.5;
...
const ORE_RENDER_STYLES = Object.freeze({
  copper: Object.freeze({ material: 'hammered-ribbon', nodeWidth: 17, nodeHeight: 11 }),
  coal: Object.freeze({ material: 'fractured-lump', nodeWidth: 16, nodeHeight: 14 }),
  iron: Object.freeze({ material: 'banded-metal', nodeWidth: 18, nodeHeight: 13 }),
  amber: Object.freeze({ material: 'resin-drop', nodeWidth: 13, nodeHeight: 17 }),
  silver: Object.freeze({ material: 'needle-thread', nodeWidth: 20, nodeHeight: 9 }),
  gold: Object.freeze({ material: 'nugget-cluster', nodeWidth: 18, nodeHeight: 14 }),
  amethyst: Object.freeze({ material: 'shard-cluster', nodeWidth: 16, nodeHeight: 19 }),
  prism_crystal: Object.freeze({ material: 'prismatic-facet', nodeWidth: 17, nodeHeight: 20 }),
  void_ore: Object.freeze({ material: 'hollow-lens', nodeWidth: 20, nodeHeight: 15 }),
...
function resetVeinSpriteCache() {
  FIELD_VEIN_SPRITE_CACHE.clear();
}

function runtimeImageHasSize(image, expected) {
  return Boolean(
    image
...
function requestFieldArtAssets() {
  if (FIELD_ART_RUNTIME.requested || typeof window.Image !== 'function') return;
  FIELD_ART_RUNTIME.requested = true;

  const terrainImage = new window.Image();
  FIELD_ART_RUNTIME.terrainImage = terrainImage;
  terrainImage.decoding = 'async';
  terrainImage.onload = () => {
    FIELD_ART_RUNTIME.terrainReady = runtimeImageHasSize(terrainImage, FIELD_TERRAIN_ATLAS_SIZE);
    if (FIELD_ART_RUNTIME.terrainReady) resetTerrainBaseCache();
  };
  ...
  oreImage.decoding = 'async';
  oreImage.onload = () => {
    FIELD_ART_RUNTIME.oreReady = runtimeImageHasSize(oreImage, FIELD_ORE_ATLAS_SIZE);
  };
  oreImage.onerror = () => {
    FIELD_ART_RUNTIME.oreReady = false;
  };
  oreImage.src = FIELD_ORE_ATLAS_SOURCE;

  const veinImage = new window.Image();
  FIELD_ART_RUNTIME.veinImage = veinImage;
  veinImage.decoding = 'async';
  veinImage.onload = () => {
    FIELD_ART_RUNTIME.veinReady = runtimeImageHasSize(veinImage, FIELD_VEIN_ATLAS_SIZE);
    resetVeinSpriteCache();
  };
  veinImage.onerror = () => {
    FIELD_ART_RUNTIME.veinReady = false;
    resetVeinSpriteCache();
  };
  veinImage.src = FIELD_VEIN_ATLAS_SOURCE;
}
...
function mirroredAtlasAxisSegments(start, length, span) {
  const segments = [];
  let consumed = 0;
  while (consumed < length) {
    const phase = positiveModulo(start + consumed, span * 2);
    const flipped = phase >= span;
    const distanceIntoHalf = flipped ? phase - span : phase;
    const sliceSize = Math.min(length - consumed, span - distanceIntoHalf);
    segments.push({
      destinationOffset: consumed,
      sourceOffset: flipped ? span - distanceIntoHalf - sliceSize : distanceIntoHalf,
      size: sliceSize,
      flipped,
    });
    consumed += sliceSize;
...
function drawMirroredAtlasRegion(renderContext, image, region, sourceX, sourceY, destinationX, destinationY, width, height) {
  const horizontalSegments = mirroredAtlasAxisSegments(sourceX, width, region.width);
  const verticalSegments = mirroredAtlasAxisSegments(sourceY, height, region.height);
  for (const vertical of verticalSegments) {
    for (const horizontal of horizontalSegments) {
      const destinationLeft = destinationX + horizontal.destinationOffset;
      const destinationTop = destinationY + vertical.destinationOffset;
      renderContext.save();
      try {
        renderContext.translate(
          destinationLeft + (horizontal.flipped ? horizontal.size : 0),
          destinationTop + (vertical.flipped ? vertical.size : 0),
        );
        renderContext.scale(horizontal.flipped ? -1 : 1, vertical.flipped ? -1 : 1);
        renderContext.drawImage(
          image,
          region.x + horizontal.sourceOffset,
          region.y + vertical.sourceOffset,
          horizontal.size,
          vertical.size,
          0,
          0,
...
function drawRuntimeTerrainTexture(renderContext, x, y, tx, ty, kind) {
  ...
  if (
    ...
    || !FIELD_ART_RUNTIME.terrainImage
    || !region
    || renderContext === ctx
    || typeof renderContext.drawImage !== 'function'
  ) return false;

  const sourceX = tx * TILE_SIZE;
  // The final seal has one authored energy seam. Lock that seam to the middle
  // of its one-tile layer while the horizontal sample still flows across the
  // complete world. Ordinary materials remain continuous in both axes.
  const sourceY = kind === 'final_seal'
    ? Math.round(region.height * 0.5 - TILE_SIZE * 0.5)
    : ty * TILE_SIZE;
  ...
  try {
    ...
    try { renderContext.restore(); } catch (_restoreError) { /* procedural fallback below */ }
    return false;
  }
}
...
function drawFracturedStratumMark(x, y, tx, ty) {
  const split = 10 + Math.floor(tileNoise(tx, ty, 175) * 5);
  const leftY = 11 + Math.floor(tileNoise(tx, ty, 174) * 7);
  const rightY = 11 + Math.floor(tileNoise(tx + 1, ty, 174) * 7);
  const splitRatio = split / TILE_SIZE;
  const splitY = leftY + (rightY - leftY) * splitRatio;
  ctx.save();
  ctx.globalAlpha = 0.24;
  ctx.strokeStyle = ty % 3 === 0 ? '#a95c3d' : '#78939a';
  ctx.lineWidth = 1.4;
  ctx.beginPath();
  ctx.moveTo(x, y + leftY);
...
function drawWorld(now) {
  ...
  // Resolve every visible ore before painting. Connector visibility therefore
  // never depends on whether its neighbour happened to be earlier or later in
  // the row-major draw order.
  const oreVisualStates = new Map();
  for (const entry of visible) {
    if (!entry.tile?.oreId) continue;
    oreVisualStates.set(
      oreVisualStateKey(entry.tx, entry.ty),
      getOreVisualState(entry.tile, entry.tx, entry.ty, entry.x, entry.y),
    );
  }
...
function drawTile({ tile, tx, ty, x, y }, now, phase = 'all', oreVisualStates = null) {
  if (!tile) return;
  const drawTerrain = phase !== 'overlay';
  const drawOverlay = phase !== 'terrain';
  if (tile.kind === 'air') {
  ...
  if (drawTerrain) {
    ...
    const authoredSeamAlpha = FIELD_ART_RUNTIME.terrainReady ? 0.32 : 1;
    ctx.save();
    ctx.globalCompositeOperation = 'lighter';
    ctx.globalAlpha = (0.28 + pulse * 0.2) * authoredSeamAlpha;
    ctx.fillStyle = '#f5c95d';
    ctx.fillRect(x, y + 12, TILE_SIZE + 1, 4);
    ctx.globalAlpha = (0.2 + hitProgress * 0.48) * authoredSeamAlpha;
  ...
  if (drawOverlay && tile.oreId) {
    ...
    const visualState = oreVisualStates?.get(oreVisualStateKey(tx, ty))
  ...
  if (drawOverlay && tile.cracked > 0.02 && tile.kind !== 'bedrock') drawCracks(x, y, tx, ty, tile.cracked);
}
...
function drawRuntimeOreNode(centerX, centerY, ore, revealed, noise, branchNoise, transform = null) {
  ...
  try {
    ...
    try { ctx.restore(); } catch (_restoreError) { /* procedural fallback below */ }
    return false;
  }
}

function getRuntimeVeinSprite(oreId) {
  const region = FIELD_VEIN_ATLAS_LAYOUT[oreId];
  if (
    !FIELD_ART_RUNTIME.veinReady
    || !FIELD_ART_RUNTIME.veinImage
    || !region
  ) return null;

  if (FIELD_VEIN_SPRITE_CACHE.has(oreId)) return FIELD_VEIN_SPRITE_CACHE.get(oreId);

  // Isolate each atlas cell in browsers that support an offscreen canvas.
  // That keeps bilinear filtering from borrowing pixels from the neighbouring
  // material. Headless/minimal DOMs safely fall back to a direct source crop.
  let sprite = null;
  try {
    if (typeof document?.createElement === 'function') {
      ...
      const surfaceContext = typeof surface.getContext === 'function'
        ? surface.getContext('2d', { alpha: true })
        : null;
      if (surfaceContext && typeof surfaceContext.drawImage === 'function') {
        surfaceContext.clearRect(0, 0, region.width, region.height);
  ...
  if (!sprite) {
    sprite = Object.freeze({
      image: FIELD_ART_RUNTIME.veinImage,
      x: region.x,
      y: region.y,
      width: region.width,
      height: region.height,
  ...
  FIELD_VEIN_SPRITE_CACHE.set(oreId, sprite);
  return sprite;
}

function oreVisualStateKey(tx, ty) {
  return `${tx}:${ty}`;
}

function veinTileIsLiveAndKnown(tx, ty, oreId, veinId, oreVisualStates = null) {
  if (!state.world || !veinId) return false;
  const tile = state.world.getTile(tx, ty);
  if (
    !tile
    || tile.kind === 'air'
  ...
  const cachedVisualState = oreVisualStates?.get(oreVisualStateKey(tx, ty));
  if (cachedVisualState) return Boolean(cachedVisualState.visible);
  const isLockedTarget = Boolean(
    (state.target?.tx === tx && state.target?.ty === ty)
    || (state.backupTarget?.tx === tx && state.backupTarget?.ty === ty)
  );
  const sensedUntil = Number(tile.sensedUntil) || 0;
  return Boolean(tile.discovered || (sensedUntil > 0 && sensedUntil >= state.elapsed) || isLockedTarget);
}

function getVeinConnectorMask(tx, ty, oreId, veinId, oreVisualStates = null) {
  if (!veinTileIsLiveAndKnown(tx, ty, oreId, veinId, oreVisualStates)) return 0;
  let mask = 0;
  if (veinTileIsLiveAndKnown(tx - 1, ty, oreId, veinId, oreVisualStates)) mask |= 1;
  if (veinTileIsLiveAndKnown(tx + 1, ty, oreId, veinId, oreVisualStates)) mask |= 2;
  if (veinTileIsLiveAndKnown(tx, ty - 1, oreId, veinId, oreVisualStates)) mask |= 4;
  if (veinTileIsLiveAndKnown(tx, ty + 1, oreId, veinId, oreVisualStates)) mask |= 8;
  return mask;
}
...
function drawRuntimeVeinEdge(edge, ore) {
  ...
  try {
    ...
    try { ctx.restore(); } catch (_restoreError) { /* keep the ore-node fallback alive */ }
    return false;
  }
}
...
function drawBeams() {
  for (const beam of state.beams) {
    ...
    if (beam.kind === 'chain') {
      ...
      ctx.stroke();
      ctx.setLineDash([]);
    } else if (beam.kind === 'sense-sector') {
...
function drawParticles() {
  for (const particle of state.particles) {
    ...
    if (particle.kind === 'spark') {
      ...
      ctx.rotate(Math.atan2(particle.vy, particle.vx));
      ctx.beginPath();
      ctx.ellipse(0, 0, particle.size * 1.8, particle.size * 0.45, 0, 0, Math.PI * 2);
      ctx.fill();
...
function bindEvents() {
  ...
  ui.soundToggle?.addEventListener('click', toggleSoundPreference);
...
window.__DEPTH_ZERO__ = {
  getSnapshot: () => ({
    ...
    liftSupplyOreId: state.liftSupplyOreId,
    deepest: state.deepest,
    descentCadence: {
      active: descentCadenceIsActive(),
      queued: state.descentCadenceQueued,
      queuedTargetKey: state.descentCadenceQueuedTargetKey,
      stalledFor: state.descentCadenceStallElapsed,
      goalDepth: state.descentCadenceGoalDepth,
      startedAt: state.descentCadenceStartedAt,
      delay: descentCadenceStallDelay(),
      pulseDuration: descentCadencePulseDuration(),
      graceRemaining: state.descentCadenceGraceRemaining,
    },
    ...
    fieldArt: {
      terrainReady: FIELD_ART_RUNTIME.terrainReady,
      oreReady: FIELD_ART_RUNTIME.oreReady,
      veinReady: FIELD_ART_RUNTIME.veinReady,
      terrainSource: FIELD_TERRAIN_ATLAS_SOURCE,
      oreSource: FIELD_ORE_ATLAS_SOURCE,
      veinSource: FIELD_VEIN_ATLAS_SOURCE,
    },
  }),
  getStats: () => ({ ...stats }),
  debugGetLiftResupplyPreferences: (tierCap = ORE_TYPES.length - 1) => (
    [...getLiftResupplyPreferences(tierCap)]
  ),
  debugSetSelectedUpgrade: (upgradeId = null) => {
    if (upgradeId && !upgradeById.has(upgradeId)) return false;
    state.selectedUpgradeId = upgradeId || null;
    return true;
  },
  ...
  getUpgradeCatalog: () => UPGRADE_DEFS.map((definition) => {
    ...
    return {
      ...
      levelSlotsRemaining: levelStatus.remaining,
  ...
  debugGetVeinConnectorMask: (tx, ty) => {
    const tileX = Math.floor(tx);
    const tileY = Math.floor(ty);
    const tile = state.world?.getTile(tileX, tileY);
  ...
  debugGetExplorationSearchMinTy: (scanRadius = EXPLORATION_SCAN_TILES, finalFallback = false) => {
    if (state.mode !== 'run' || !state.player || !state.world) return null;
    const center = state.world.worldToTile(state.player.x, state.player.y);
    return explorationSearchMinTy(center.ty, scanRadius, Boolean(finalFallback));
  },
  debugGetActiveDepthFloorTy: () => (
    state.mode === 'run' && state.player && state.world
      ? activeDepthFloorTy()
      : null
  ),
  ...
  debugSetDescentCadenceState: (patch = {}) => {
    if (Object.hasOwn(patch, 'deepest')) state.deepest = Math.max(0, Number(patch.deepest) || 0);
    if (Object.hasOwn(patch, 'anchorDepth')) {
      state.descentCadenceAnchorDepth = Math.max(0, Number(patch.anchorDepth) || 0);
    }
    if (Object.hasOwn(patch, 'stallElapsed')) {
      state.descentCadenceStallElapsed = Math.max(0, Number(patch.stallElapsed) || 0);
    }
    if (Object.hasOwn(patch, 'goalDepth')) {
      state.descentCadenceGoalDepth = Math.max(0, Number(patch.goalDepth) || 0);
    }
    if (Object.hasOwn(patch, 'startedAt')) {
      state.descentCadenceStartedAt = Number.isFinite(Number(patch.startedAt))
        ? Number(patch.startedAt)
        : -1;
    }
    if (Object.hasOwn(patch, 'graceRemaining')) {
      state.descentCadenceGraceRemaining = Math.max(0, Number(patch.graceRemaining) || 0);
    }
    if (Object.hasOwn(patch, 'queued')) state.descentCadenceQueued = Boolean(patch.queued);
    if (Object.hasOwn(patch, 'queuedAt')) {
      state.descentCadenceQueuedAt = Number.isFinite(Number(patch.queuedAt))
        ? Number(patch.queuedAt)
        : -1;
    }
    if (Object.hasOwn(patch, 'queuedTargetKey')) {
      state.descentCadenceQueuedTargetKey = patch.queuedTargetKey
        ? String(patch.queuedTargetKey)
        : null;
    }
    return window.__DEPTH_ZERO__.getSnapshot().descentCadence;
  },
  debugAdvanceDescentCadence: (seconds = 0) => {
  ...
  debugAutoBuyAffordable: (limit = 200) => {
    ...
    for (let purchase = 0; purchase < purchaseLimit; purchase += 1) {
      ...
      const pendingProgressTool = CAMPAIGN_PROGRESS_TOOLS
        .map((id) => upgradeById.get(id))
        .find((definition) => (
          definition
          && getUpgradeLevel(definition) < campaignAutobuyTargetLevel(definition)
          && upgradeIsAvailable(definition)
        ));
      const pendingProgressToolRecipe = pendingProgressTool
        ? getUpgradeRecipe(pendingProgressTool, getUpgradeLevel(pendingProgressTool))
        : {};
      const pendingStrategic = UPGRADE_DEFS.filter((definition) => {
        const targetLevel = campaignAutobuyTargetLevel(definition);
        return getUpgradeLevel(definition) < targetLevel
          && upgradeIsPurchaseEligible(definition);
      });
      const reservedOreIds = new Set();
      for (const definition of pendingStrategic) {
        const level = getUpgradeLevel(definition);
        const recipe = getUpgradeRecipe(definition, level);
        for (const oreId of Object.keys(recipe)) reservedOreIds.add(oreId);
      }
      ...
      const candidates = UPGRADE_DEFS
        .filter((definition) => {
          ...
          const recipe = getUpgradeRecipe(definition, level);
          if (!canAffordRecipe(save.inventory, recipe)) return false;
          if (
            pendingProgressTool
            && definition.id !== pendingProgressTool.id
            && Object.entries(pendingProgressToolRecipe).some(([oreId, reservedAmount]) => (
              (recipe[oreId] || 0) > Math.max(
                0,
                (save.inventory[oreId] || 0) - reservedAmount,
              )
            ))
          ) return false;
          const strategicTarget = campaignAutobuyTargetLevel(definition);
          const isPendingStrategic = level < strategicTarget;
          const reservesFinalMechanicSlot = lastFirstRankSlot
            && level === 0
            && BREAKTHROUGH_FIRST_RANK_IDS.has(definition.id)
            && !CAMPAIGN.capstones.includes(definition.id);
          if (
            !isPendingStrategic
            && !reservesFinalMechanicSlot
            && Object.keys(recipe).some((oreId) => reservedOreIds.has(oreId))
          ) return false;
          return true;
        })
        .map((definition) => {
          ...
          const strategicWeight = level < strategicTarget
            ? (
              CAMPAIGN_AUTOBUY_WEIGHTS[definition.id]
              || (CAMPAIGN_FINAL_PATH_TARGETS.has(definition.id) ? 0.14 : 1)
            )
            : 1;
    ...
    if (!stats.oreFocusUnlocked) {
      ...
      const focusCandidates = pendingStrategic.length
        ? pendingStrategic
        : UPGRADE_DEFS;
      for (const definition of focusCandidates) {
        const level = getUpgradeLevel(definition);
      ...
      const focus = ORE_TYPES
        ...
        .sort((left, right) => (
          deficits[right.id] * (right.value || 1) - deficits[left.id] * (left.value || 1)
          || right.tier - left.tier
        ))[0];
  ...
  debugTriggerDeepResonance: () => {
    state.target = null;
    state.backupTarget = null;
    state.deepResonanceCooldownRemaining = 0;
    return triggerDeepResonance(getFocusedOre());
  },
